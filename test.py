...
if args.mode[0] == "run":
    ...
    for target in targets:
        ...
        try:
            ...
            sys.stderr.write(
                "Missing target data for domain %(domain)s. Run `./test.py update %(domain)s` to correct this, "
                "after verifying that wizard_whois can correctly parse this particular domain.\n" % {
                    "domain": target})
            errors = True
            continue
    ...
    for target, data, target_default, target_normalized in suites:
        for normalization in (True, []):
            ...
            parsed = wizard_whois.parse.parse_raw_whois(data, normalized=normalization)
    ...
    for target, data in updates:
        default = wizard_whois.parse.parse_raw_whois(data)
        normalized = wizard_whois.parse.parse_raw_whois(data, normalized=True)
